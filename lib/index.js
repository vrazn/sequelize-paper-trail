...
exports.init = (sequelize, optionsArg) => {
	...
	const defaultOptions = {
		...
		userModel: false,
		adminModel: false,
		adminModelAttribute: 'adminId',
		userModelAttribute: 'userId',
		actionModelAttribute: 'actionId',
		actionModel: false,
		enableCompression: false,
		enableMigration: false,
		enableStrictDiff: true,
		continuationNamespace: null,
		continuationAdminKey: 'adminId',
		continuationActionKey: 'actionId',
	...
	function createAfterHook(operation) {
		const afterHook = function afterHook(instance, opt) {
			...
			if (
				opt.context &&
				((opt.context.delta && opt.context.delta.length > 0) ||
					destroyOperation)
			) {
				...
				if (!destroyOperation && options.enableCompression) {
					_.forEach(opt.fields, (a) => {
						...
						if (
							!_.isEqual(
								instance._previousDataValues[a],
								instance.dataValues[a],
							)
						) {
							currentVersion[a] = instance.dataValues[a];
						}
				...
				query[options.adminModelAttribute] =
					(ns && ns.get(options.continuationAdminKey)) || opt.adminId;
				query[options.actionModelAttribute] =
					(ns && ns.get(options.continuationActionKey)) ||
					opt.actionId;
	...
	return {
		...
		defineModels: function defineModels(db) {
			...
			Revision.associate = function associate(models) {
				...
				Revision.belongsTo(
					sequelize.model(options.adminModel),
					options.belongsToAdminOptions,
				);
				Revision.belongsTo(
					sequelize.model(options.actionModel),
					options.belongsToActionOptions,
				);
			};
